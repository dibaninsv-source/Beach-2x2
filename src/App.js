...
const LANG = {
  ru: {
    ...
    save: "✓ Сохранить матч", sendReview: "⏳ Отправить на проверку",
    ...
    winRate: "побед",
    gamesLabel: (n) => n === 1 ? "игра" : n < 5 ? "игры" : "игр",
    ...
    deleteTitle: "Удалить игрока?", deleteDesc: "будет удалён из команды", deleteBtn: "Удалить",
    photoTitle: "Фото профиля", uploadPhoto: "📷 Загрузить фото", deletePhoto: "🗑 Удалить фото",
  ...
  lv: {
    ...
    p1: "Spēlētājs 1", p2: "Spēlētājs 2",
    noTie: "⚠️ Neizšķirts nav atļauts",
    save: "✓ Saglabāt spēli", sendReview: "⏳ Nosūtīt pārbaudei",
    pendingTag: "⏳ Pārbaude",
    ...
    winRate: "uzvaras",
    gamesLabel: (n) => n === 1 ? "spēle" : "spēles",
    rankGames: "🏆 Games", rankGamesHint: "Kurš uzvarēja vairāk spēļu",
    rankKing: "👑 King", rankKingHint: "Kurš ir labāks ±punktos",
    period: "PERIODS", periodHint: "· ℹ️ dienas detaļas",
    noPlayers: "Pievieno spēlētājus cilnē «Spēle»",
    roster: "👥 KOMANDAS SASTĀVS",
    trophyTitle: "🏆 Apbalvojumu dēlis",
    gamesWinner: "= Games uzvarētājs", kingWinner: "= King uzvarētājs",
    noAwards: "nav apbalvojumu",
    dayResults: "DIENAS REZULTĀTI",
    victory: "🏆 UZVARA",
    adminMode: "🔑 Admin režīms", viewMode: "👁 Skatīties",
    ...
    deleteTitle: "Dzēst spēlētāju?", deleteDesc: "tiks dzēsts no komandas", deleteBtn: "Dzēst",
    photoTitle: "Profila foto", uploadPhoto: "📷 Augšupielādēt foto", deletePhoto: "🗑 Dzēst foto",
  ...
  en: {
    ...
    save: "✓ Save match", sendReview: "⏳ Send for review",
    pendingTag: "⏳ Pending",
    ...
    deleteTitle: "Delete player?", deleteDesc: "will be removed from team", deleteBtn: "Delete",
    photoTitle: "Profile photo", uploadPhoto: "📷 Upload photo", deletePhoto: "🗑 Remove photo",
...
function computeStats(players, matches) {
  ...
  matches.forEach(m => {
    [{ pair:[m.t1p1,m.t1p2], sc:m.score1, cn:m.score2 },
     { pair:[m.t2p1,m.t2p2], sc:m.score2, cn:m.score1 }].forEach(({ pair, sc, cn }) => {
      pair.forEach(pid => {
        if (!st[pid]) return;
...
function computeTrophies(players, matches) {
  ...
  days.forEach(day => {
    ...
    const byDiff = [...players].sort((a,b) => {
      const da = st[a.id].scored - st[a.id].conceded;
      const db2 = st[b.id].scored - st[b.id].conceded;
      return db2 !== da ? db2 - da : st[b.id].scored - st[a.id].scored;
    });
    ...
    for (let k=0;k<n;k++) for (let l=k+1;l<n;l++)
      if (i!==k && i!==l && j!==k && j!==l) {
        const key = [[players[i].id,players[j].id].sort().join("-"),
                     [players[k].id,players[l].id].sort().join("-")].sort().join("|");
...
// ── Shared styles ─────────────────────────────────────────────────────────────
const cardStyle = { background:C.card, borderRadius:14, padding:14, border:`1px solid ${C.border}`, marginBottom:10 };
const inpStyle = { background:C.cardHi, border:`1px solid ${C.border}`, borderRadius:9, padding:"9px 12px",
  color:C.text, fontFamily:"'Outfit',sans-serif", fontSize:14, outline:"none", width:"100%", boxSizing:"border-box" };
const btnStyle = (bg, sm) => ({ background:bg, border:"none", borderRadius:9,
  padding: sm ? "7px 14px" : "10px 18px", color:"#fff", fontWeight:700,
  fontFamily:"'Outfit',sans-serif", fontSize: sm ? 13 : 14, cursor:"pointer" });
const lblStyle = { fontSize:11, color:C.sub, fontWeight:700, letterSpacing:1.2, marginBottom:6 };
const selStyle = { background:C.cardHi, border:`1px solid ${C.border}`, borderRadius:9, padding:"8px 10px",
...
// ── AdminPopup ────────────────────────────────────────────────────────────────
function AdminPopup({ t, onSuccess, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:300,
      ...
      <div style={{background:"#161626",borderRadius:18,padding:24,width:"100%",maxWidth:300,
        ...
        <div style={{fontSize:15,fontWeight:800,textAlign:"center",color:C.text,marginBottom:18}}>{t.adminTitle}</div>
        <input autoFocus type="password" placeholder={t.adminPw} value={pw}
          onChange={e => { setPw(e.target.value); setErr(false); }}
          onKeyDown={e => { if (e.key==="Enter") try_(); if (e.key==="Escape") onClose(); }}
          style={{...inpStyle, border:`1.5px solid ${err ? "#FF6B6B" : C.border}`, marginBottom:6}} />
        {err && <div style={{fontSize:12,color:"#FF6B6B",marginBottom:6}}>{t.wrongPw}</div>}
        <div style={{display:"flex",gap:8,marginTop:12}}>
          <button onClick={onClose} style={{...btnStyle(C.cardHi),flex:1,color:C.sub,border:"1px solid "+C.border}}>{t.cancel}</button>
          <button onClick={try_} style={{...btnStyle(C.sand),flex:2,color:"#0D0D1A"}}>{t.enter}</button>
        </div>
      </div>
    </div>
  );
...
// ── EditMatchPopup ────────────────────────────────────────────────────────────
function EditMatchPopup({ t, match, players, onSave, onClose }) {
    ...
    <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:300,
      ...
      <div style={{background:"#161626",borderRadius:16,padding:18,width:"100%",maxWidth:360,
        ...
        <div style={{background:C.cardHi,borderRadius:10,padding:10,marginBottom:8,borderLeft:"3px solid "+C.sky}}>
          <div style={{...lblStyle,color:C.sky,marginBottom:6}}>{t.team1}</div>
          <div style={{display:"flex",gap:6}}>
            <select style={{...selStyle,flex:1}} value={t1p1} onChange={e => setT1p1(e.target.value)}>
              {players.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <select style={{...selStyle,flex:1}} value={t1p2} onChange={e => setT1p2(e.target.value)}>
        ...
        <div style={{background:C.cardHi,borderRadius:10,padding:10,marginBottom:12,borderLeft:"3px solid "+C.red}}>
          <div style={{...lblStyle,color:C.red,marginBottom:6}}>{t.team2}</div>
          <div style={{display:"flex",gap:6}}>
            <select style={{...selStyle,flex:1}} value={t2p1} onChange={e => setT2p1(e.target.value)}>
              {players.filter(p => ![t1p1,t1p2].includes(p.id)).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <select style={{...selStyle,flex:1}} value={t2p2} onChange={e => setT2p2(e.target.value)}>
              {players.filter(p => ![t1p1,t1p2,t2p1].includes(p.id)).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:16,marginBottom:14}}>
          <ScoreInput val={s1} setVal={setS1} color={C.sky} label={t.team1} />
          <div style={{fontSize:20,fontWeight:900,color:C.sub,paddingTop:18}}>:</div>
          <ScoreInput val={s2} setVal={setS2} color={C.red} label={t.team2} />
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={onClose} style={{...btnStyle(C.cardHi),flex:1,color:C.sub,border:"1px solid "+C.border}}>{t.cancel}</button>
          <button onClick={() => valid && onSave({...match,t1p1,t1p2,t2p1,t2p2,score1:parseInt(s1),score2:parseInt(s2)})}
            style={{...btnStyle(valid ? C.sand : C.border),flex:2,color:valid?"#0D0D1A":C.sub,opacity:valid?1:0.6}}>
            {t.save}
          </button>
        </div>
      </div>
...
// ── DayDetailPopup ────────────────────────────────────────────────────────────
function DayDetailPopup({ t, day, matches, players, onClose }) {
  const dm = matches.filter(m => m.date === day);
          ...
          return (
            <div key={p.id} style={{display:"flex",alignItems:"center",gap:8,
              ...
              <div style={{display:"flex",gap:10,fontSize:12}}>
                <span style={{color:C.green}}>{s.wins}{t.wins[0]}</span>
                <span style={{color:C.red}}>{s.losses}{t.losses[0]}</span>
...
// ── TrophyBoard ───────────────────────────────────────────────────────────────
function TrophyBoard({ t, players, trophies, onClose }) {
    ...
    <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:300,
      ...
      <div style={{background:"#161626",borderRadius:"16px 16px 0 0",padding:"18px 16px 30px",
        ...
        <div style={{display:"flex",justifyContent:"center",gap:16,marginBottom:14,fontSize:11,color:C.sub}}>
          <span>🏆 {t.gamesWinner}</span><span>👑 {t.kingWinner}</span>
        </div>
        {sorted.map((p, i) => {
          ...
          return (
            <div key={p.id} style={{display:"flex",alignItems:"center",gap:10,
              ...
              <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",justifyContent:"flex-end"}}>
                {tr.cups>0 && <div>{"🏆".repeat(Math.min(tr.cups,5))}{tr.cups>5&&<span style={{fontSize:11,color:C.sand}}>×{tr.cups}</span>}</div>}
                {tr.crowns>0 && <div>{"👑".repeat(Math.min(tr.crowns,5))}{tr.crowns>5&&<span style={{fontSize:11,color:C.king}}>×{tr.crowns}</span>}</div>}
...
// ── PhotoPickerPopup ──────────────────────────────────────────────────────────
function PhotoPickerPopup({ t, player, onSave, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:400,
      ...
      <div style={{background:"#161626",borderRadius:18,padding:24,width:"100%",maxWidth:300,
        ...
        <button onClick={() => fileRef.current.click()} style={{...btnStyle(C.accent),width:"100%",marginBottom:10}}>{t.uploadPhoto}</button>
        {player.photo && (
          <button onClick={() => { onSave(null); onClose(); }}
            style={{...btnStyle(C.cardHi),width:"100%",color:C.red,border:"1px solid "+C.border,marginBottom:10}}>
            {t.deletePhoto}
          </button>
        )}
        <button onClick={onClose} style={{...btnStyle(C.cardHi),width:"100%",color:C.sub,border:"1px solid "+C.border}}>{t.cancel}</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const u3 = onSnapshot(collection(db,"dayLinks"), snap => {
      const links = {};
  ...
  const fbAddPlayer = async name => { const id = uid(); await setDoc(doc(db,"players",id),{name,photo:null,createdAt:Date.now()}); };
  const fbUpdatePlayer = async (id, data) => { await updateDoc(doc(db,"players",id),data); };
  const fbDeletePlayer = async id => { await deleteDoc(doc(db,"players",id)); };
  const fbAddMatch = async data => { const id = uid(); await setDoc(doc(db,"matches",id),{...data,id,createdAt:Date.now()}); };
  const fbUpdateMatch = async (id, data) => { await updateDoc(doc(db,"matches",id),data); };
  const fbDeleteMatch = async id => { await deleteDoc(doc(db,"matches",id)); };
  const fbSetDayLink = async (date, url) => { await setDoc(doc(db,"dayLinks",date),{url}); };
  ...
  const sortedPlayers = useMemo(() => [...players].sort((a,b) => {
    ...
    if (rankMode==="games") {
      ...
      const da = sa.scored-sa.conceded, db2 = sb.scored-sb.conceded;
      return db2!==da ? db2-da : sb.scored-sa.scored;
    }
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",width:"100%",maxWidth:430,margin:"0 auto",
      ...
      {showAdminPopup && <AdminPopup t={t} onSuccess={()=>{setIsAdmin(true);setShowAdminPopup(false);}} onClose={()=>setShowAdminPopup(false)} />}
      {editingMatch && <EditMatchPopup t={t} match={editingMatch} players={players}
        onSave={async u=>{await fbUpdateMatch(u.id,u);setEditingMatch(null);}}
        onClose={()=>setEditingMatch(null)} />}
      {dayDetailDay && <DayDetailPopup t={t} day={dayDetailDay} matches={matches} players={players} onClose={()=>setDayDetailDay(null)} />}
      {showTrophies && <TrophyBoard t={t} players={players} trophies={trophies} onClose={()=>setShowTrophies(false)} />}
      {photoPickerPlayer && <PhotoPickerPopup t={t} player={photoPickerPlayer}
      ...
      {deleteConfirm && (
        <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:400,
          ...
          <div style={{background:"#161626",borderRadius:16,padding:22,width:"100%",maxWidth:300,
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setDeleteConfirm(null)} style={{...btnStyle(C.cardHi),flex:1,color:C.sub,border:"1px solid "+C.border}}>{t.cancel}</button>
              <button onClick={async()=>{await fbDeletePlayer(deleteConfirm);setSessionIds(ids=>ids.filter(id=>id!==deleteConfirm));setDeleteConfirm(null);}}
                style={{...btnStyle(C.red),flex:1}}>{t.deleteBtn}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{flex:1,padding:"12px 12px 80px",overflowY:"auto"}}>
        ...
        {tab==="game" && (<>
          <div style={cardStyle}>
            <div style={lblStyle}>{t.addPlayer}</div>
            <div style={{display:"flex",gap:8}}>
              <input style={{...inpStyle,flex:1}} placeholder={t.playerPlaceholder} value={newName}
              ...
              <button style={btnStyle(C.accent)} onClick={()=>{const n=newName.trim();if(n&&!players.find(p=>p.name.toLowerCase()===n.toLowerCase())){fbAddPlayer(n);setNewName("");}}}>+</button>
            </div>
          </div>

          {players.length>0 && (
            <div style={cardStyle}>
              <div style={lblStyle}>{t.playing} ({sessionIds.length})
                {!isAdmin && <span style={{color:C.sub,fontWeight:400,marginLeft:6,fontSize:10}}>· {t.viewOnly}</span>}
              </div>
          ...
          {suggestions.length>0 && (
            <div style={cardStyle}>
              <div style={lblStyle}>{t.suggestions}</div>
          ...
          <div style={{...cardStyle,border:"1px solid "+(isAdmin?C.sand+"30":C.pending+"30")}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div style={lblStyle}>{t.recordMatch}</div>
            ...
            <div style={{marginBottom:10}}>
              <div style={{...lblStyle,marginBottom:4}}>{t.date}</div>
              <input type="date" style={inpStyle} value={matchDate} onChange={e=>setMatchDate(e.target.value)} />
            </div>
            <div style={{background:C.cardHi,borderRadius:10,padding:10,marginBottom:8,borderLeft:"3px solid "+C.sky}}>
              <div style={{...lblStyle,color:C.sky}}>{t.team1}</div>
              <div style={{display:"flex",gap:6}}>
                <select style={{...selStyle,flex:1}} value={t1p1} onChange={e=>setT1p1(e.target.value)}>
            ...
            <div style={{background:C.cardHi,borderRadius:10,padding:10,marginBottom:14,borderLeft:"3px solid "+C.red}}>
              <div style={{...lblStyle,color:C.red}}>{t.team2}</div>
              <div style={{display:"flex",gap:6}}>
                <select style={{...selStyle,flex:1}} value={t2p1} onChange={e=>setT2p1(e.target.value)}>
                  <option value="">{t.p1}</option>
            ...
            <button style={{...btnStyle(matchValid?(isAdmin?C.sand:C.pending):C.border),width:"100%",
              color:matchValid?"#0D0D1A":C.sub,opacity:matchValid?1:0.6}} onClick={saveMatch}>
              {isAdmin ? t.save : t.sendReview}
            </button>
          </div>

          {players.length>0 && (
            <div style={cardStyle}>
              <div style={{...lblStyle,marginBottom:10}}>{t.roster}</div>
                ...
                {players.map(p => (
                  <div key={p.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,width:56}}>
                    <div style={{position:"relative"}}>
                      ...
                      {isAdmin && (
                        <div style={{position:"absolute",bottom:-2,right:-2,background:C.accent,
        ...
        {tab==="stats" && (<>
          <div style={{...cardStyle,padding:"10px 12px"}}>
            ...
            <div style={lblStyle}>{t.period} <span style={{fontWeight:400,fontSize:10,color:C.sub}}>{t.periodHint}</span></div>
          ...
          {filteredMatches.length>0 && (
            <div style={{...cardStyle,background:C.cardHi,padding:"10px 16px",
          ...
          {sortedPlayers.map((p, i) => {
            ...
            const bigLabel = isKing ? t.pm : t.wins;
            ...
            return (
              <div key={p.id} style={{...cardStyle,border:i===0?`1px solid ${isKing?C.king:C.green}55`:"1px solid "+C.border}}>
                ...
                <div style={{display:"flex",justifyContent:"space-around",marginTop:12,
                  ...
                  {[
                    [st.wins, t.wins, C.green],
                    [st.losses, t.losses, C.red],
                    [st.games, t.games, C.sub],
                    [st.scored, t.scored, C.sky],
                    [st.conceded, t.conceded, C.sub],
        ...
        {tab==="history" && (<>
          {isAdmin && pendingCount>0 && (
            <div style={{...cardStyle,background:C.pending+"18",border:"1px solid "+C.pending+"55",
              ...
              <button onClick={approveAll} style={{...btnStyle(C.pending,true),color:"#fff"}}>{t.approveAll}</button>
            </div>
          )}
                    ...
                    editingLinkDay===day ? (
                      <div style={{display:"flex",gap:4,alignItems:"center"}}>
                        <input autoFocus value={linkInput} onChange={e=>setLinkInput(e.target.value)}
                          ...
                          style={{...inpStyle,padding:"4px 8px",fontSize:11,width:160,borderRadius:8}} />
                        <button onClick={async()=>{await fbSetDayLink(day,linkInput.trim());setEditingLinkDay(null);}}
                          style={{...btnStyle(C.green,true),padding:"4px 8px",fontSize:11}}>✓</button>
                        <button onClick={()=>setEditingLinkDay(null)}
                          style={{...btnStyle(C.cardHi,true),padding:"4px 8px",fontSize:11,color:C.sub,border:"1px solid "+C.border}}>✕</button>
                      </div>
                    ) : (
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                        ...
                        {isAdmin && (
                          <div style={{display:"flex",gap:6,alignItems:"center"}}>
                            {m.pending && (
                              <button onClick={()=>fbUpdateMatch(m.id,{pending:false})}
                                style={{...btnStyle(C.green,true),fontSize:10,padding:"3px 10px"}}>{t.approveOne}</button>
                            )}
